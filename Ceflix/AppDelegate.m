...
- (BOOL)application:(UIApplication *)application openURL:(NSURL *)url sourceApplication:(nullable NSString *)sourceApplication annotation:(id)annotation
{
        ...
        NSString *query = [url lastPathComponent];
        
        NSDictionary *dict =@{@"eventID":query};
    
    [self performSelector:@selector(callnotification:) withObject:dict afterDelay:0.2];
...
-(void)callnotification:(NSDictionary *)dict
{
     [[NSNotificationCenter defaultCenter] postNotificationName:@"open" object:self userInfo:dict];
}

- (void)application:(UIApplication *)application didRegisterUserNotificationSettings:(nonnull UIUserNotificationSettings *)notificationSettings
{
    NSLog(@"Registering device for push notification...");
