...
- (NSString *)fetchLiveVideosSuccess:(void (^)(NSArray *))success failure:(CFCeflixServiceFailure)failure {
    ...
    return [self submitGETPath:@"/videos/watchlivecatalog" success:^(NSData *data) {
        ...
        NSArray *results = [resultDictionary objectForKey:CFDataVideosKey];
          NSArray *live = [resultDictionary objectForKey:@"angles"];
        if (results && [results isKindOfClass:[NSArray class]]) {
            NSArray *liveVideos = [results mappedArrayWithBlock:^id(id obj) {
        return [[CFCeflixRemoteVideo alloc] initWithDictionary:obj];
            }];
            
            NSArray *livestreaming=@[];
            if (live && [live isKindOfClass:[NSArray class]])
            {
    livestreaming = [live mappedArrayWithBlock:^id(id obj) {
                    return [[CFCeflixRemoteVideo alloc] initWithLiveStreamDictionary:obj];
                }];
            }
            if (success != NULL)
            {
                NSArray *array=@[liveVideos,livestreaming];
                success(array);
            }
        }
        else if(live && [live isKindOfClass:[NSArray class]])
        {
            NSArray *livestreaming = [live mappedArrayWithBlock:^id(id obj) {
                return [[CFCeflixRemoteVideo alloc] initWithLiveStreamDictionary:obj];
            }];
            
            if (success != NULL)
            {
                NSArray *array=@[@[],livestreaming];
...
- (NSString *)fetchCommentsForLiveStreamVideo:(NSDictionary *)dict success:(void (^)(NSArray *))success failure:(CFCeflixServiceFailure)failure {
    self.serverRoot = [NSURL URLWithString:@"http://apix3x9.ceflix.org"];
    return [self submitPOSTPath:@"/angels/getVideoComments" body:dict expectedStatus:200 success:^(NSData *data) {
        NSError *error = nil;
        NSDictionary *resultDictionary = [NSJSONSerialization JSONObjectWithData:data options:0 error:&error];
        NSArray *results = [resultDictionary objectForKey:@"data"];
        if (results && [results isKindOfClass:[NSArray class]]) {
...
-(void)webservicescall:(NSDictionary*)bodydict action:(NSString *)action block:(ServiceCall)block
{
    ...
    NSString *string=[NSString stringWithFormat:@"http://apix3x9.ceflix.org/%@",action];
    
    
   NSMutableURLRequest *request= [self requestForURL:[NSURL URLWithString:string] method:@"POST" bodyDict:bodydict];
